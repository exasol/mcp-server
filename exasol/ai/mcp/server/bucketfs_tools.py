...
class BucketFsTools:
    ...
    def _list_items(self, rel_dir: str, item_filter: Callable[[bfs.path.PathLike], bool]) -> ExaDbResult:
        ...
        content = [
            ...
            for pth in abs_dir.iterdir() if item_filter(pth)
        ]
        return ExaDbResult(content)

    def list_directories(self, rel_dir: str = '') -> ExaDbResult:
        """
        Lists subdirectories at the given directory. The directory path is relative
        to the root location.
        """
        return self._list_items(rel_dir, lambda pth: pth.is_dir())

    def list_files(self, rel_dir: str = '') -> ExaDbResult:
        """
        Lists files at the given directory. The directory path is relative to the
        root location.
        """
        return self._list_items(rel_dir, lambda pth: pth.is_file())

    def find_files(self, keywords: list[str], rel_path: str = '') -> ExaDbResult:
        """
