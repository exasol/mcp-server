...
development. Exasol MCP Server will likely fall behind at certain points.
...
environment variables, as described in the documentation for a particular provider.
...
| EXA_AUTH_TOKEN_ENDPOINT_AUTH_METHOD      |    no    | Token endpoint authentication method for upstream server. <br/>Common values: "client_secret_basic","client_secret_post", "none". <br/>If not set, the provider will user default (typically "client_secret_basic").                                                                                          |
